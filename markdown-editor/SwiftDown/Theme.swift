...
public struct Theme {
  ...
  public init(_ name: String) {
    ...
    let themeDirectory = URL(fileURLWithPath: #file).deletingLastPathComponent().appendingPathComponent("Resources/Themes")
    let themePath = themeDirectory.appendingPathComponent("\(name).json").path

    guard FileManager.default.fileExists(atPath: themePath) else {
        print("[SwiftDown] Unable to load your theme file: \(name).json")
        return
    }

    self.init(themePath: themePath)
  }
